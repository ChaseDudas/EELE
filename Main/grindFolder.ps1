function grindFolder()
{
    ...
    #{
        Copy-Item $zipfileFolder -Destination "D:\Excel Link Grinder\Back_Up_Rate_Files" -Recurse 
    #}
    #else
    #{
    #    Write-Warning "Folder already exists in backups`n"
    #    exit
    #}
    ...
    Foreach($zippieFile in (Get-ChildItem $zipfileFolder -Include *.xlsx -Recurse))
    {
        ...
        {
            Write-Warning "There are no External Links in this file `n" 
            ...
            Rename-Item -Path $zipfileName -NewName $zipLeaf
